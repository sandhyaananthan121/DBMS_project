...
const dbPath = './database/sandhyaananthan.db'; // Replace with your .db file's name
...
app.get('/total', async (req, res) => {
    ...
    db.all(query, (err, rows) => {
      ...
      const results = rows.map(rows => Object.values(rows));
      res.json(results);
    //await connection.close();
    //res.json(result.rows);
  //} catch (error) {
...
app.get('/q1', async (req, res) => {
    ...
    const startDateFormatted = startDate.split('-').join('-'); // Assuming startDate is 'YYYY-MM-DD'
    const endDateFormatted = endDate.split('-').join('-');     // Assuming endDate is 'YYYY-MM-DD'

  // Execute the query
    db.all(query, [startDateFormatted, endDateFormatted, selectedCountry, sector], (err, rows) => {
    if (err) {
        res.status(500).json({ error: err.message });
        return;
    }
    ...
    res.json(results);
      ...
      // Modify your SQL query using the extracted parameters
      // const result = `SELECT value, date_recorded 
      // FROM co2_sectorwise 
      // WHERE DATE(date_recorded, 'DD/MM/YYYY') BETWEEN DATE(?, 'DD/MM/YYYY') AND DATE(?, 'DD/MM/YYYY') 
      // AND country=? 
      // AND sector =?
      // `;
      // db.all(result, [startDate, endDate, selectedCountry, sector], (err, rows) => {
      //   if (err) {
      //     res.status(500).json({ error: err.message });
      //     return;
      //   }
  ...
  app.get('/q1/country', async (req, res) => {
    //try {
      // Your code to fetch data from Oracle DB
      //const connection = await oracledb.getConnection(dbConfig);
      const result = 'select distinct(country) from co2_sectorwise';
  ...
  app.get('/q1/sector', async (req, res) => {
    //try {
      // Your code to fetch data from Oracle DB
      //const connection = await oracledb.getConnection(dbConfig);
      const result = 'select distinct(sector) from co2_sectorwise';
      db.all(result, (err, rows) => {
        if (err) {
          res.status(500).json({ error: err.message });
          return;
  ...
  app.get('/q1/main', async (req, res) => {
    ...
    where  country=? 
      AND sector = ?
      `;
      db.all(query,  [selectedCountry, sector],  (err, rows) => {
        if (err) {
          res.status(500).json({ error: err.message });
          return;
        }
  ...
  app.get('/q1/side', async (req, res) => {
    ...
    AND year = ?
      `;
      db.all(query,  [selectedCountry,selectedYear ], (err, rows) => {
        if (err) {
          res.status(500).json({ error: err.message });
          return;
        }
...
//Q2
  ...
  app.get('/q2', async (req, res) => {
    //try {
      ...
      let {val} = req.query;
      //const connection = await oracledb.getConnection(dbConfig);
      let query = `WITH PopulationGrowth AS (
        SELECT
            P1.COUNTRY,
            P1.YEAR,
            P1.VALUE AS POPULATION,
...
CO2_EMISSION - LAG(CO2_EMISSION) OVER (PARTITION BY COUNTRY ORDER BY YEAR) AS CO2Change
    ...
    AND CO2Change IS NOT NULL -- Ensure non-null CO2 emission change values
      ...
      `; db.all(query, [selectedCountry, selectedStartYear, selectedEndYear],(err, rows) => {
        if (err) {
          res.status(500).json({ error: err.message });
          return;
        }
...
//     `; 
    ...
    SELECT 
        CG.COUNTRY,
        CG.YEAR,
        SUM(CG.VALUE) AS TotalCO2_EMISSION,
        SUM(ET.EMISSIONS_TAXED) AS TotalEMISSIONS_TAXED
    FROM
        CO2_GREENHOUSE CG
    JOIN
        ENVIRONMENTAL_TAX ET 
    ON 
        CG.COUNTRY = ET.COUNTRY AND CG.YEAR = ET.YEAR
    GROUP BY
        CG.COUNTRY, CG.YEAR
),
RateChange AS (
    SELECT
        ...
        TotalCO2_EMISSION,
        TotalEMISSIONS_TAXED,
        TotalCO2_EMISSION - LAG(TotalCO2_EMISSION) OVER (PARTITION BY COUNTRY ORDER BY YEAR) AS CO2_EMISSION_CHANGE,
        TotalEMISSIONS_TAXED - LAG(TotalEMISSIONS_TAXED) OVER (PARTITION BY COUNTRY ORDER BY YEAR) AS TAX_CHANGE
    FROM
        CO2TaxData
)
SELECT 
...
FROM 
    RateChange
WHERE 
    CO2_EMISSION_CHANGE IS NOT NULL AND TAX_CHANGE IS NOT NULL
ORDER BY 
    COUNTRY, YEAR;
`;
      db.all(query, (err, rows) => {
...
RateChange AS (
    SELECT
        ...
        TotalCO2_EMISSION,
        TotalEMISSIONS_TAXED,
        -- Calculate change in CO2 emissions from the previous year
        TotalCO2_EMISSION - LAG(TotalCO2_EMISSION) OVER (PARTITION BY COUNTRY ORDER BY YEAR) AS CO2_EMISSION_CHANGE,
        -- Calculate change in emissions taxed from the previous year
        TotalEMISSIONS_TAXED - LAG(TotalEMISSIONS_TAXED) OVER (PARTITION BY COUNTRY ORDER BY YEAR) AS TAX_CHANGE
    FROM
        CO2TaxData
)
...
WHERE 
    ...
    AND CO2_EMISSION_CHANGE IS NOT NULL
    AND TAX_CHANGE IS NOT NULL
ORDER BY 
    YEAR;
`
    ...
    //try {
      ...
      const query = `WITH CO2_GDP AS (
        ...
        FROM
          ...
          INNER JOIN GDP ON CG.COUNTRY = GDP.COUNTRY AND CG.YEAR = GDP.YEAR
        WHERE
          CG.YEAR between ? and ? -- Filter by selected year range
          and cg.Country = ? -- Filter by selected country
        GROUP BY
          CG.COUNTRY, CG.YEAR
      ),
 -- Calculate Normalized Aggregate Rate and CO2 Score    
    ...
    -- Calculate the final aggregate value (Normalized Aggregate Rate * CO2 Score)
      ...
      `;
      db.all(query, [selectedStartYear, selectedEndYear, selectedCountry ],(err, rows) => {
        if (err) {
          res.status(500).json({ error: err.message });
          return;
        }
  ...
  app.get('/q5/main', async (req, res) => {
    //try {
      ...
      SELECT Year, TOTAL_CO2_EMISSIONS
      FROM FINALDATA
      WHERE TEMP_RANGE=? -- Input drop down - Freezing, Cold, Mild, Warm, Hot
      and CO2_RANGE=?   -- Input drop down - Low, Average, High
      and country=?
      `;
      db.all(query,[ selectedTempRange, selectedCo2Range, selectedCountry], (err, rows) => {
        if (err) {
          res.status(500).json({ error: err.message });
          return;
        }
